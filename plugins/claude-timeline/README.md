...
|---------|--------------|
| `/claude-timeline:timeline` | Start the local server (default port `7373`) and open the dashboard. Idempotent — re-running just reopens the tab. |
| `/claude-timeline:timeline-stop` | Stop the server. |
| `/claude-timeline:timeline-setup` | Install the statusLine wrapper so the plugin captures rate-limit data first-party. Detects any existing statusLine command and chains through it. |
| `/claude-timeline:timeline-setup uninstall` | Remove the wrapper, restore your original `statusLine` command. |
| `/claude-timeline:timeline-autostart enable` | Install a launchd (macOS) or systemd-user (Linux) unit so the server starts at login and auto-restarts on crash. |
| `/claude-timeline:timeline-autostart disable` | Remove the unit. Manual `/claude-timeline:timeline` still works. |
| `/claude-timeline:timeline-autostart status` | Show whether autostart is installed and active. |
...
If you don't run `/claude-timeline:timeline-setup`, the plugin falls back to other data sources automatically (see below).
...
| `"self"` | Only use our own cache. Requires `/claude-timeline:timeline-setup`. Most accurate. |
...
| `downstream_statusline` | `null` | A statusLine command to chain through after capturing. Auto-populated by `/claude-timeline:timeline-setup` when an existing statusLine is detected. Set manually if you want to pipe to a custom renderer. If `null`, the wrapper captures silently and the visible status bar is blank. |
...
Restart the server (`/claude-timeline:timeline-stop && /claude-timeline:timeline`) after editing.

## Lifecycle

By default the server is **not** a system service — it's started by `/claude-timeline:timeline` and runs as a detached `nohup` process.

| Event | Without autostart | With `/claude-timeline:timeline-autostart enable` |
|-------|-------------------|-----------------------------------|
...
| `/claude-timeline:timeline-stop` | Server stops. Won't restart until you run `/claude-timeline:timeline` again. | Server stops; **stays stopped** for the rest of the session (graceful exit), but the launchd/systemd unit will restart it on the next login. |
| Server crashes | Stays down until you run `/claude-timeline:timeline`. | Auto-restarts (`KeepAlive` with `SuccessfulExit=false` on macOS; `Restart=on-failure` on Linux). |
| System restart | Server gone; run `/claude-timeline:timeline` from inside Claude Code to bring it back. | Server starts automatically at next login. |
...
- **Before `/claude-timeline:timeline-setup`:** zero coupling — we read its cache as a fallback.
- **After `/claude-timeline:timeline-setup`:** the setup script detects `cs render` (or whatever you have) and saves it as `downstream_statusline`. Our wrapper captures the stdin, then pipes it to `cs render`. Both tools keep their own cache. The visible status bar in your terminal continues to work exactly as before.
- **To revert:** `/claude-timeline:timeline-setup uninstall` puts your original `statusLine.command` back.
...
| `~/.claude/settings.json` | Modified by `/claude-timeline:timeline-setup` (`statusLine.command` key only) |
...
| `~/Library/LaunchAgents/com.blaze.claude-timeline.plist` *(macOS)* | Autostart unit (only if `/claude-timeline:timeline-autostart enable`) |
| `~/.config/systemd/user/claude-timeline.service` *(Linux)* | Autostart unit (only if `/claude-timeline:timeline-autostart enable`) |

## Troubleshooting

**Dashboard shows "no data" / "heuristic" source.**
You haven't run `/claude-timeline:timeline-setup` and claude-statusbar isn't installed either. Either run `/claude-timeline:timeline-setup` (recommended) or install claude-statusbar. The plugin still works in heuristic mode — just less precise.

**Wrong session window / numbers flicker between two sets of values.**
Probably had multiple Claude Code windows open with stale per-session caches. The plugin filters out caches whose `resets_at` is in the past, but if you see this on the latest version, file an issue. Workaround: run `/claude-timeline:timeline-setup` to switch to first-party capture (a single source of truth).

**Server won't start / port in use.**
Change `port` in `~/.claude/claude-timeline.json` and restart, or kill whatever's holding `7373` (`lsof -i :7373`).

**`/claude-timeline:timeline` says "already running" but no browser tab opens.**
Open `http://localhost:7373` manually. macOS `open` sometimes silently no-ops if your default browser is in a weird state.

**Visible status bar disappeared after `/claude-timeline:timeline-setup`.**
...
```
/claude-timeline:timeline-autostart disable        # if you enabled it
/claude-timeline:timeline-setup uninstall          # restore original statusLine
/claude-timeline:timeline-stop                     # kill server
...
| `scripts/server.mjs`, `analyze.mjs`, `config.mjs` | ❌ Restart server (`/claude-timeline:timeline-stop && /claude-timeline:timeline`) |
